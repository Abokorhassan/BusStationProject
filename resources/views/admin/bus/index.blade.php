                        ...
                        @if(!empty($buses))
                            @foreach ($buses as $bus)
                                <tr>
                                    ...
                                    <td>{{ $bus->Driver_id }}</td>
                                    <td>{{ $bus->station_id}}</td>
                                    {{-- <td>{{ $test->comments->count() }}</td> --}}
                                    <td>{{ $bus->created_at->diffForHumans() }}</td>
                                    <td>
                                       
                                        <a href="{{ URL::to('admin/bus/' . $bus->id . '/edit' ) }}"><i class="livicon"
                                                                                                         data-name="edit"
                                                                                                         data-size="18"
                                                                                                         data-loop="true"
                                                                                                         data-c="#428BCA"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="user_delete_confirm_title">Delete bus</h4>
            </div>
            <div class="modal-body">
                          {{-- @if($error)
                              <div>{!! $error !!}</div>
                          ...
                          @endif --}}
                Are you sure to delete this Bus? This operation is irreversible
                          
            </div>
            <div class="modal-footer">
                <button type="button" style="margin-right: 1%" class="btn btn-default" data-dismiss="modal">Cancel</button>
                {!!Form::open(['url' => ['admin/bus', $bus->id], 'method' => 'Post', 'class' => 'pull-right']) !!}
                    {{Form::hidden('_method', 'DELETE')}}
                    {{Form::submit('Delete', ['class' => 'btn btn-danger'])}}
                {!!Form::close() !!}
            </div>
