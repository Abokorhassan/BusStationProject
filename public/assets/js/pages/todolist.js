...
$(document).ready(function () {
    ...
    $('.add_button').bind('click',function(e){
        ...
        $.ajax({
            type: "POST",
            url: url+ "/admin/task/create",
            data: $("form#main_input_box").serialize(),
            success: function (id) {
                var count = $('#taskcount').text();
                count = parseInt(count) + 1;
                $(".list_of_items").prepend("<div class='todolist_list showactions list1' id='" + id + "'>  " + "<div class='col-md-8 col-sm-8 col-xs-8 nopadmar pull-left custom_textbox1'> <div class='todoitemcheck'>" + checkBox + "</div>" + "<div class='todotext todoitemjs'>" + $("#task_description").val() + " </div> <div class='date'> <span class='label label-default end_date'>" + $("#task_deadline").val() + "</span></div></div>" + twoButtons);
    ...
    $.ajax({
        type: "GET",
        url: url+ "/admin/task/data",
        success: function (result) {
            $.each(result, function (i, item) {
                $('.list_of_items').append("<div class='todolist_list showactions list1' id='" + item.id + "'>   " +
                    "<div class='col-md-8 col-sm-8 col-xs-8 nopadmar pull-left custom_textbox1'> <div class='todoitemcheck'>" + "<p><input type='checkbox' class='striked ' autocomplete='off' " + ((item.finished == 1) ? "checked" : "") + "/></p>" +
